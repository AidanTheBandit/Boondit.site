...
export const fetchMetadata = async (elements) => {
  ...
  try {
    ...
    for (const proxyUrl of proxies) {
      try {
        ...
        if (htmlContent) {
          ...
          if (
            doc.body &&
            (doc.querySelector("title") || doc.querySelector("head"))
          ) {
    ...
    if (!htmlContent || !doc) {
      throw (
        lastError ||
        new Error("All proxy services failed to return valid content")
      );
    }
    ...
    if (metadata.description && elements.description)
    ...
    if (metadata.iconUrl && elements.iconUrl) {
      try {
        ...
        elements.iconUrl.value = metadata.iconUrl;
      }
    }
    if (metadata.author && elements.author)
